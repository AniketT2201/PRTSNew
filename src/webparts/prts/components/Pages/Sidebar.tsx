...
const Sidebar = (props: IPrtsProps) => {
  ...
  const [isACLUser, setIsACLUser] = React.useState(false);
  const [canCreateRequest, setCanCreateRequest] = React.useState(false);
...
async function GetIAS_ACL() {
  ...
  // Find all ACL entries for current user
  const userACL = Momentflowdata.filter(
    item =>
      item.UserName?.EMail?.trim().toLowerCase() ===
      props.userEmail?.trim().toLowerCase()
  );

  // User exists in ACL?
  setIsACLUser(userACL.length > 0);

  // User has Employee ID?
  const hasEmployeeId = userACL.some(
    item => item.EmployeeID && String(item.EmployeeID).trim() !== ""
  );

  setCanCreateRequest(hasEmployeeId);

  // Keep only matching EmployeeID record for role lookup
  const Userfiltereddata = userACL.filter(
    item => item.EmployeeID === props.EmployeeId?.[0]?.EmployeeID
  );
