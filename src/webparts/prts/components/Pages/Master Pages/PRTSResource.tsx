...
export const PRTSResource: React.FC<IPrtsProps> = (props: IPrtsProps) => {

    const [searchTerm, setSearchTerm] = useState("");
    const [PRTSResourceData, setPRTSResourceData] = useState<any[]>([]);
    const [filteredData, setFilteredData] = useState<any[]>([]);
    const [popupVisible, setPopupVisible] = useState(false);
    const [loading, setLoading] = useState(true);
    const [recordsPerPage, setRecordsPerPage] = useState(10); // default 10 records per page
    const [originalPRTSResourceTitle, setOriginalPRTSResourceTitle] = useState("");
    ...
    const GetPRTSResourceData = async () => {
        let employeeProfile = await EmployeeProfile(props.userEmail);
        try {
            setLoading(true);
            const PRTSResource = await MasterPagesRequestsOps().getPRTSResourceData(
                { column: "ID", isAscending: false },
                props,
                ''
            );
        
            //console.log('PRTS Resource data: ', PRTSResource);// Debug log
            setPRTSResourceData(PRTSResource);
            setFilteredData(PRTSResource);
        } catch (err: any) {
            console.error(err.message);
            alert("Error fetching PRTS Resource data. Please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        if (!searchTerm) {
            setFilteredData(PRTSResourceData);
        } else {
            const lowerSearch = searchTerm.toLowerCase();
            const filtered = PRTSResourceData.filter(item =>
                (item.Title || "").toLowerCase().includes(lowerSearch) 
            );
            setFilteredData(filtered);
            setCurrentPage(1);
        }
    }, [searchTerm, PRTSResourceData]);  // ✅ added PRTSResourceData
    ...
    const exportToExcel = () => {
        ...
        const exportData = dataToExport.map((item) => ({
            "PRTS Resource Name": item.Title
        }));

        // Create sheet + workbook
        const worksheet = XLSX.utils.json_to_sheet(exportData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "PRTSResource");

        // Save file with today’s date
        const today = new Date().toISOString().slice(0, 10); // yyyy-mm-dd
        XLSX.writeFile(workbook, `PRTSResource_${today}.xlsx`);
    };
    ...
    // store original PRTS Resource Name
    setOriginalPRTSResourceTitle(item.Title);
    setVendorForm({
        Title: item.Title
    });
    setPopupVisible(true);
    ...
    const validateForm = () => {
      if (!vendorForm.Title) {
        alert("PRTS Resource Name is required");
        return false;
      }
    ...
    const update = async () => {
        ...
        setLoading(true);
        try {
            // 🔹 Check duplicate PRTSResourceName
            // if (vendorForm.Title !== originalPRTSResourceTitle) {
              const title = vendorForm.Title.trim();
              const vendorData = await MasterPagesRequestsOps().getPRTSResourceData(
                  { column: "ID", isAscending: true },
                  props,
            ...
            await spCrudObj.updateData('PRTSResource', selectedId, vendorForm, props);
            alert("PRTS Resource details updated successfully!");
            } else {
            await spCrudObj.insertData('PRTSResource', vendorForm, props);
            alert("PRTS Resource details added successfully!");
            }

            setPopupVisible(false);
            GetPRTSResourceData(); // refresh grid
        } catch (err: any) {
            console.error(err.message);
            alert("Error saving PRTS Resource details. Please try again.");
        } finally {
            setLoading(false); 
        }
    };
            ...
            <div className="header">
                <div className="left-banner">
                    <div className="logo-text">
                        <h2>PRTS Resource Master Dashboard</h2>
                    </div>
                </div>
            </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 0' }}>
                          <div className="flex flex-col">
                            ...
                            <button className="btn btn-warning export-btn" onClick={openAddPopup} style={{ marginLeft: "10px" }}>
                                <i className="fa fa-mail-forward"></i>Add PRTS Resource
                            </button>
                        </div>
                        {/* Table */}
                                ...
                                <table className="min-w-full bg-white rounded-2xl shadow-md">
                                    <thead style={{ backgroundColor: "#ce0b0e", position: "sticky", top: "0px" }} className="text-white">
                                        <tr>
                                            ...
                                            <th className="px-4 py-2">PRTS Resource Name</th>
                                        </tr>
                                    ...
                                    <div className="modal-dialog modal-lg">
                                        <div className="modal-content">
                                            <div className="modal-body">
                                                <h4 className="modal-title">{isEdit ? "Edit PRTS Resource" : "Add PRTS Resource"}</h4>
