...
export const BuildType: React.FC<IPrtsProps> = (props: IPrtsProps) => {

    const [searchTerm, setSearchTerm] = useState("");
    const [BuildTypeData, setBuildTypeData] = useState<any[]>([]);
    const [filteredData, setFilteredData] = useState<any[]>([]);
    const [popupVisible, setPopupVisible] = useState(false);
    const [loading, setLoading] = useState(true);
    const [recordsPerPage, setRecordsPerPage] = useState(10); // default 10 records per page
    const [originalBuildTypeTitle, setOriginalBuildTypeTitle] = useState("");
    ...
    const GetBuildTypeData = async () => {
        let employeeProfile = await EmployeeProfile(props.userEmail);
        try {
            setLoading(true);
            const BuildType = await MasterPagesRequestsOps().getBuildTypeData(
                { column: "ID", isAscending: false },
                props,
                ''
            );
        
            //console.log('Build Type data: ', BuildType);// Debug log
            setBuildTypeData(BuildType);
            setFilteredData(BuildType);
        } catch (err: any) {
            console.error(err.message);
            alert("Error fetching Build Type data. Please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        if (!searchTerm) {
            setFilteredData(BuildTypeData);
        } else {
            const lowerSearch = searchTerm.toLowerCase();
            const filtered = BuildTypeData.filter(item =>
                (item.Title || "").toLowerCase().includes(lowerSearch) 
            );
            setFilteredData(filtered);
            setCurrentPage(1);
        }
    }, [searchTerm, BuildTypeData]);  // ✅ added BuildTypeData
    ...
    const exportToExcel = () => {
        ...
        const exportData = dataToExport.map((item) => ({
            "Build Type Name": item.Title
        }));

        // Create sheet + workbook
        const worksheet = XLSX.utils.json_to_sheet(exportData);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "BuildType");

        // Save file with today’s date
        const today = new Date().toISOString().slice(0, 10); // yyyy-mm-dd
        XLSX.writeFile(workbook, `BuildType_${today}.xlsx`);
    };
    ...
    // store original Build Type Name
    setOriginalBuildTypeTitle(item.Title);
    setVendorForm({
        Title: item.Title
    });
    setPopupVisible(true);
    ...
    const validateForm = () => {
      if (!vendorForm.Title) {
        alert("Build Type Name is required");
        return false;
      }
        ...
        try {
            ...
            // if (vendorForm.Title !== originalBuildTypeTitle) {
              ...
              if (vendorData.length > 0) {
                  alert("Build Type Name already exists!");
                  setLoading(false);
                  return;
              }
            // }  
            ...
            await spCrudObj.insertRootData('BuildType', vendorForm, props);
            alert("Build Type details added successfully!");
            }

            setPopupVisible(false);
            GetBuildTypeData(); // refresh grid
        } catch (err: any) {
            console.error(err.message);
            alert("Error saving Build Type details. Please try again.");
        } finally {
            setLoading(false); 
        }
    };
            ...
            <div className="header">
                <div className="left-banner">
                    <div className="logo-text">
                        <h2>Build Type Master Dashboard</h2>
                    </div>
                </div>
            </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 0' }}>
                          <div className="flex flex-col">
                            ...
                            <button className="btn btn-warning export-btn" onClick={openAddPopup} style={{ marginLeft: "10px" }}>
                                <i className="fa fa-mail-forward"></i>Add Build Type
                            </button>
                        </div>
                        {/* Table */}
                                ...
                                <table className="min-w-full bg-white rounded-2xl shadow-md">
                                    <thead style={{ backgroundColor: "#ce0b0e", position: "sticky", top: "0px" }} className="text-white">
                                        <tr>
                                            ...
                                            <th className="px-4 py-2">Build Type Name</th>
                                        </tr>
                                    ...
                                    <div className="modal-dialog modal-lg">
                                        <div className="modal-content">
                                            <div className="modal-body">
                                                <h4 className="modal-title">{isEdit ? "Edit Build Type" : "Add Build Type"}</h4>
