...
export const AllReqDash: React.FC<IPrtsProps> = (props: IPrtsProps) => {
    ...
    const exportToExcel = () => {
        ...
        // Convert filtered data into plain array of objects
        ...
        saveAs(data, `Filtered_Requests_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };
                                        ...
                                        {[...filteredData].sort((a, b) => b.ID - a.ID).slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
                                            .map((item, index) => (
                                                <tr key={index} className="border-t">
                                                    <td className="px-4 py-2"><a onClick={(e) => { e.preventDefault(); handleTitleClick(item.ID); }} href="#" className="text-blue-600 hover:text-blue-800 underline">{item.ReqNo}</a></td>
