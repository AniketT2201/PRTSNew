...
const handleBaseInfoSaveDraft = async (formState: any) => {
  ...
  if (cleanedForm.mRepeatedIssue === "Yes") {
    ...
    if (!refNo) {
      alert(
        "Reference Request Number is mandatory for Repeated Issue."
      );
      return;
    }
    ...
    if (!exists) {
      alert(
        `Reference Request No '${refNo}' does not exist in the system.`
      );
      return;
    }
  }
  ...
  updateSummary(
    ...
    isNewDraft ? "Draft Created" : "Draft Updated",
    ""
  );
  ...
  const updateData: any = {
    ...
    PurgingAttachment:
      cleanedForm.selectedFiles?.[0]?.name || "",
...
const uploadAttachments = async (itemId: number, files?: File[]) => {
  ...
  //     const summary = JSON.stringify(jsonSummary);
  //     const userId = props.currentSPContext.pageContext.legacyPageContext.userId;
  //     const addResult = await sp.web.lists.getByTitle("PRTSList").items.add({ Status: "Draft", Summary: summary, InitiatorId: userId, Stage: 0, InitDepartment: EmployeeId?.DepartmentCode?.Department, InitiatorEmpId: props.EmployeeId[0].EmployeeID });
